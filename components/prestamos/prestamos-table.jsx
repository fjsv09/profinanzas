...
export default function PrestamosTable({ 
  ...
  // Función para manejar la búsqueda local si no se proporciona una función externa
  ...
  // Filtrar préstamos localmente si no se proporciona función externa
  const filteredPrestamos = onSearch ? prestamos : prestamos.filter(prestamo =>
  ...
  const puedeEditar = (prestamo) => {
    ...
    if (usuario.rol === 'supervisor') {
      ...
      return asesoresSupervisa.includes(prestamo.cliente.asesor_id);
    }
  ...
  const getEstadoPrestamo = (estado) => {
    switch (estado) {
      case 'activo':
      ...
      <div className="mb-4 flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4">
        ...
        <div className="md:w-64">
          <select
            ...
            onChange={(e) => onFiltroEstadoChange && onFiltroEstadoChange(e.target.value)}
          >
            ...
            {filteredPrestamos.length > 0 ? (
              filteredPrestamos.map((prestamo) => (
                <tr key={prestamo.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {getEstadoPrestamo(prestamo.estado)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-right">
                    ...
                    {puedeEliminar(prestamo) && (
                      <button
                        onClick={() => onDelete && onDelete(prestamo.id)}
